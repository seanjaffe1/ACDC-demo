"""
Training script for the gauge-equivariant flow matching model.

Identical to train_main.py except:
  - Uses GaugeGraphEncoder (GaugeEGNN backbone) instead of GraphEncoder
  - Loads graphs from data/graphs_curvature/ (in_node_dim=4 curvature features)
  - Checkpoint cfg includes model_type='gauge' so eval scripts can
...
Usage:
    ...
    python -m src.training.train_gauge --config configs/gauge_model.yaml \\
        --set kl_weight=0.005 out_dir=checkpoints/gauge_kl5e-3
"""
...
def train(cfg: dict):
    ...
    run_name = cfg.get("wandb_run_name", "gauge-egnn-flow")
    ...
    run = wandb.init(
        ...
        config=cfg,
        tags=["gauge", "egnn", "flow-matching", "curvature"],
    )
    ...
    for epoch in range(1, cfg["epochs"] + 1):
        ...
        for batch in train_loader:
            ...
            z = GaugeGraphEncoder.reparameterise(mu, logvar)
        ...
        if val_loss < best_val_loss:
            ...
            # Include model_type in cfg so eval scripts know which encoder to use
            save_cfg = dict(cfg)
            save_cfg["model_type"] = "gauge"
            torch.save({
...
def main():
    ...
    parser.add_argument("--config", default="configs/gauge_model.yaml")
    parser.add_argument(
